#ifndef _FBCM_SiPadDigitizerAlgorithm_CommonDigiUtility_h
#define _FBCM_SiPadDigitizerAlgorithm_CommonDigiUtility_h
...
namespace CommonDigiUtility {
  ...
  public:
    ...
    Amplitude(float amp, const PSimHit* hitp, float frac = 0, float tcor = 0, size_t hitIndex = 0, uint32_t tofBin = 0)
      ...
      if (frac > 0) {
        if (hitp != nullptr)
          _simInfoList.push_back({frac, std::make_unique<PSimHitInfo>(hitp, tcor, hitIndex, tofBin)});
        else
    ...
    const std::vector<std::pair<float, std::unique_ptr<PSimHitInfo> > >& simInfoList() const { return _simInfoList; }
    ...
    void operator+=(const Amplitude& other) {
      ...
      for (auto const& ic : other.simInfoList()) {
        if (ic.first > -0.5)
          _simInfoList.push_back({ic.first, std::make_unique<PSimHitInfo>(*ic.second)});
      }
    }
  ...
  private:
    float _amp;
    std::vector<std::pair<float, std::unique_ptr<PSimHitInfo> > > _simInfoList;
  };
